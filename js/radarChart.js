...
function loadMetaData(sample) {
  d3.csv("ScoreInfo.csv").then((data) => {
    var result = data.find((row) => row.City === sample);
    console.log(result);

    var PANEL = d3.select(".card-text");

    PANEL.html("");

    Object.entries(result).forEach(([key, value]) => {
      PANEL.append("div").text(`${key}: ${Math.round(value * 100) / 100}`);
    });
  });
}
